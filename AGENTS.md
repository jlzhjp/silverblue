...
GNOME defaults are provided through keyfiles in `dconf/db/local.d/` plus `dconf/profile/user`; keep `dconf update` in the image build so `/etc/dconf/db/local` is compiled.
