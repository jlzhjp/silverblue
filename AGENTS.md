...
When publishing compressed images, keep the expensive `rpm-ostree compose build-chunked-oci` and `zstd:chunked` upload to a single content tag and move additional tags by manifest copy. The rechunk step should consume the local Buildah result, not a stable registry tag, so pull requests can validate chunking without publishing. The built image is assumed to contain `rpm-ostree` and `skopeo`; do not add package installation to the chunking step unless that assumption changes. Do not pass a fully qualified `ghcr.io/...` image name together with `registry: ghcr.io` to `redhat-actions/push-to-registry`, because that produces duplicated destinations such as `ghcr.io/ghcr.io/...`.
