...
CI builds pull requests without publishing, except PRs labeled `skip-ci`. The build job runs in `quay.io/fedora/fedora-bootc:latest` with privileged container options so `rpm-ostree compose build-chunked-oci --bootc --format-version=2` can rechunk the local Buildah image directly. Pushes to `main` and manual runs publish the Fedora version tag from `Containerfile`, `latest`, and `sha-<short-sha>` tags. CI pushes only the rechunked `sha-<short-sha>` image with `zstd:chunked` compression, then retags that manifest as the Fedora version tag and `latest` with `skopeo copy` so layers are not recompressed per tag. After publishing, CI deletes older GHCR container package versions and keeps only the 5 most recent images.
...
When publishing compressed images, keep the expensive `rpm-ostree compose build-chunked-oci` and `zstd:chunked` upload to a single content tag and move additional tags by manifest copy. The rechunk step should consume the local Buildah result, not a stable registry tag or an exported source archive, so pull requests can validate chunking without publishing. The `quay.io/fedora/fedora-bootc:latest` job container is assumed to contain `rpm-ostree` and `skopeo`; do not add package installation to the chunking step unless that assumption changes. Do not pass a fully qualified `ghcr.io/...` image name together with `registry: ghcr.io` to `redhat-actions/push-to-registry`, because that produces duplicated destinations such as `ghcr.io/ghcr.io/...`.
